...
- Aadhaar verification's on-chain step (`markAadhaarVerified`) is an owner/verifier-controlled flag, not on-chain proof verification — the frontend integrates the real Anon Aadhaar SDK to generate a proof, and the backend (`app/api/verify-aadhaar/route.js`) independently re-verifies the zk-SNARK proof and enforces one-wallet-per-nullifier before calling `markAadhaarVerified`, but the contract itself does not verify the proof.
- Aadhaar verification runs in Anon Aadhaar's **test mode** by default (`ANON_AADHAAR_USE_TEST_MODE`/`NEXT_PUBLIC_ANON_AADHAAR_USE_TEST_MODE`, both default `true` unless explicitly set to `"false"`), meaning the frontend widget only ever generates proofs against Anon Aadhaar's built-in test/dummy identity, not a real UIDAI-issued Aadhaar QR. The zk verification and on-chain call are both genuinely executed, not stubbed out — only the identity being proven is a sanctioned test one. Both flags must be flipped to `"false"` together for production Aadhaar to work at all, since a proof generated under one mode fails verification under the other (see `src/utils/aadhaarConfig.js`).
...
- **Anon Aadhaar client/backend test-mode flag mismatch fixed.** `app/providers.js` hardcoded the client-side `AnonAadhaarProvider`'s `_useTestAadhaar` prop to `true`, ignoring `NEXT_PUBLIC_ANON_AADHAAR_USE_TEST_MODE` entirely — even though `src/utils/aadhaarConfig.js`'s own comments (and its mismatch guard) assumed the client widget read that env var. In practice this meant the frontend could never generate a proof against a real UIDAI Aadhaar pubkey no matter how the env vars were set, and flipping the backend to production mode would have just made every proof fail verification (client proofs would still be test-mode, checked against the production key). `providers.js` now derives `_useTestAadhaar` from `NEXT_PUBLIC_ANON_AADHAAR_USE_TEST_MODE` with the same default-to-test-mode convention as the backend flag, so both sides can actually agree.
